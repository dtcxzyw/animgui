...
namespace animgui {
    ...
    ANIMGUI_API void property(canvas& canvas, std::pmr::string label, float& val, float min, float max, float step,
                              float smooth_step);
    ANIMGUI_API void slider(canvas& canvas, std::pmr::string label, int32_t& val, int32_t min, int32_t max);
    ANIMGUI_API void slider(canvas& canvas, std::pmr::string label, float& val, float min, float max);
    ANIMGUI_API void spinner(canvas& canvas, std::pmr::string label, int32_t& val, int32_t min, int32_t max, int32_t step);
    ANIMGUI_API void spinner(canvas& canvas, std::pmr::string label, float& val, float min, float max, float step);
