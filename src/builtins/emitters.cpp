...
namespace animgui {
    class builtin_emitter final : public emitter {
        static constexpr float padding = 3.0f;  // TODO: style
        static vec2 calc_bounds(const button_base& item) {
            return { item.content_size.x + 2 * padding, item.content_size.y + 2 * padding };
        }
        static void emit(const button_base& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = bounds{ item.anchor.x, item.anchor.y, item.anchor.x + item.content_size.x + 2 * padding,
                                item.anchor.y + item.content_size.y + 2 * padding };
        ...
        static vec2 calc_bounds(const canvas_stroke_rect& item) {
        ...
        static void emit(const canvas_stroke_rect& item, const bounds& clip_rect, std::pmr::vector<command>& commands,
                         style& style, const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = bounds{ item.bounds.left - item.size / 2.0f, item.bounds.right + item.size / 2.0f,
        ...
        static vec2 calc_bounds(const canvas_fill_rect& item) {
            return { item.bounds.right - item.bounds.left, item.bounds.bottom - item.bounds.top };
        }
        static void emit(const canvas_fill_rect& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
        ...
        static vec2 calc_bounds(const canvas_line& item) {
            return { std::fabsf(item.start.x - item.end.x) + item.size, std::fabsf(item.start.y - item.end.y) + item.size };
        }
        static void emit(const canvas_line& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = bounds{ std::fminf(item.start.x, item.end.x) - item.size / 2.0f,
        ...
        static vec2 calc_bounds(const canvas_point& item) {
            return { item.size, item.size };
        }
        static void emit(const canvas_point& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = bounds{ item.pos.x - item.size / 2.0f, item.pos.x + item.size / 2.0f, item.pos.y - item.size / 2.0f,
        ...
        static vec2 calc_bounds(const canvas_image& item) {
            return { item.bounds.right - item.bounds.left, item.bounds.bottom - item.bounds.top };
        }
        static void emit(const canvas_image& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = item.bounds;
            if(!clip_bounds(rect, clip_rect))
                return;
        ...
        static vec2 calc_bounds(const canvas_text& item) {
            float width = 0.0f;
        ...
        static void emit(const canvas_text& item, const bounds& clip_rect, std::pmr::vector<command>& commands, style& style,
                         const std::function<texture_region(font&, uint32_t)>& font_callback) {
            auto rect = clip_rect;
            auto beg = item.str.begin();
            const auto end = item.str.end();
    ...
    public:
        ...
        vec2 calculate_bounds(const primitive& primitive) override {
            return std::visit([](auto&& item) { return builtin_emitter::calc_bounds(item); }, primitive);
        }
